...
class TextLogger(Callback):
    def __init__(self, batch_frequency, max_texts, clamp=True, increase_log_steps=True,
                 rescale=True, disabled=False, log_on_batch_idx=False, log_first_step=False,
                 log_texts_kwargs=None):
        super().__init__()
        self.rescale = rescale
        self.batch_freq = batch_frequency
        self.max_texts = max_texts
        self.logger_log_texts = {
        ...
        self.log_texts_kwargs = log_texts_kwargs if log_texts_kwargs else {}
        self.log_first_step = log_first_step
    
    @rank_zero_only
    def _tensorboard(self, pl_module, texts, batch_idx, split):
        pass
    
    @rank_zero_only
    def log_local(self, save_dir, split, texts,
                  global_step, current_epoch, batch_idx):
        root = os.path.join(save_dir, "texts", split)
        for k in texts:
            filename = "{}_gs-{:06}_e-{:06}_b-{:06}".format(
                k,
                global_step,
                current_epoch,
                batch_idx)
    ...
    def log_txt(self, pl_module, batch, batch_idx, split="train"):
        check_idx = batch_idx if self.log_on_batch_idx else pl_module.global_step
        if (self.check_frequency(check_idx) and  # batch_idx % self.batch_freq == 0
                hasattr(pl_module, "log_texts") and
                callable(pl_module.log_texts) and
                batch_idx > 5 and
                self.max_texts > 0):
            ...
            with torch.no_grad():
                texts = pl_module.log_texts(batch, split=split, **self.log_texts_kwargs)
            
            for k in texts:
                N = min(len(texts[k]), self.max_texts)
                texts[k] = texts[k][:N]
            
            self.log_local(pl_module.logger.save_dir, split, texts,
                           pl_module.global_step, pl_module.current_epoch, batch_idx)
            
            logger_log_texts = self.logger_log_texts.get(logger, lambda *args, **kwargs: None)
            logger_log_texts(pl_module, texts, pl_module.global_step, split)
            if is_train:
                pl_module.train()
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        if not self.disabled and (pl_module.global_step > 0 or self.log_first_step):
            self.log_txt(pl_module, batch, batch_idx, split="train")

    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        if not self.disabled and pl_module.global_step > 0:
            self.log_txt(pl_module, batch, batch_idx, split="val")
